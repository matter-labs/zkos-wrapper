...
#[derive(Default)]
pub struct Mersenne2ndExtConvertor;

impl<F: SmallField> FnOnce<([F; 2],)> for Mersenne2ndExtConvertor {
    type Output = Mersenne31Complex;

    extern "rust-call" fn call_once(self, args: ([F; 2],)) -> Self::Output {
        let value_c0 = args.0[0].as_u64();
...
impl<F: SmallField> CSWitnessable<F, 2> for MersenneComplex<F> {
    type ConversionFunction = Mersenne2ndExtConvertor;

    fn witness_from_set_of_values(values: [F; 2]) -> Self::Witness {
        let value_c0 = values[0].as_u64();
