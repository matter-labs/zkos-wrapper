...
#[derive(Default)]
pub struct MersenneConvertor;

impl<F: SmallField> FnOnce<([F; 1],)> for MersenneConvertor {
    type Output = Mersenne31Field;

    extern "rust-call" fn call_once(self, args: ([F; 1],)) -> Self::Output {
        let value = args.0[0].as_u64();
...
impl<F: SmallField> CSWitnessable<F, 1> for MersenneField<F> {
    type ConversionFunction = MersenneConvertor;

    fn witness_from_set_of_values(values: [F; 1]) -> Self::Witness {
        let value = values[0].as_u64();
        assert!(value < M31_MODULUS);
        Mersenne31Field::new(value as u32)
    }
