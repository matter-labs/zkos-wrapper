...
pub(crate) fn transform_degree_2_constraint(
                ...
                quote! {
                    {
                        ...
                        let b = #read_b_expr;
                        a = a.mul(cs, &b);
                        #individual_term_ident = #individual_term_ident.add(cs, &a);
                    }
                }
            } else if coeff == Mersenne31Field::MINUS_ONE {
                quote! {
                    {
                        ...
                        let b = #read_b_expr;
                        a = a.mul(cs, &b);
                        #individual_term_ident = #individual_term_ident.sub(cs, &a);
                    }
                }
            } else {
                let coeff = coeff.to_reduced_u32();
    ...
    if constant_term.is_zero() == false {
        ...
        let t = quote! {
            let constant_term = MersenneField::allocate_constant(cs, Mersenne31Field(#constant_term));
            #individual_term_ident = #individual_term_ident.add_base(cs, &constant_term);
        };
            ...
            quote! {
                {
                    let mut a = #read_a_expr;
                    let coeff = MersenneField::allocate_constant(cs, Mersenne31Field(#coeff));
                    #individual_term_ident = a.mul_by_base_and_add(cs, &coeff, & #individual_term_ident);
                }
            }
        };
...
pub(crate) fn transform_degree_1_constraint(
    ...
    if linear_terms.len() == 0 {
        ...
        let stream = quote! {
            let #individual_term_ident = MersenneField::allocate_constant(cs, Mersenne31Field(#constant_term));
        };
...
pub(crate) fn transform_width_1_range_checks_pair(
    ...
    let common_stream = if add_timestamp_contribution == false {
        ...
        quote! {
            let a = #a_stream;
            let mut b = #b_stream;
            b = b.sub_base(cs, & #memory_timestamp_high_from_sequence_idx_ident);
            let c = #c_expr;
        }
    };
    ...
    let t0 = quote! {
        let #individual_term_ident = {
            ...
            #individual_term_ident = #individual_term_ident.mul(cs, &b);
            #individual_term_ident = #individual_term_ident.sub(cs, &c);
    ...
    let t1 = quote! {
        let #individual_term_ident = {
            ...
            denom = denom.mul(cs, & #lookup_argument_gamma_ident);
        ...
        for i in 0..bound {
            ...
            let t = quote! {
                let #individual_term_ident = {
                    ...
                    let mut t = #lookup_argument_linearization_challenges_ident[1];
                    t = t.mul(cs, & #src_2_expr);
                    denom = denom.add(cs, &t);

                    let mut t = #lookup_argument_linearization_challenges_ident[0];
                    t = t.mul(cs, & #src_1_expr);
                    denom = denom.add(cs, &t);
...
pub(crate) fn transform_delegation_ram_conventions(
    ...
    assert_eq!(bound, memory_layout.batched_ram_accesses.len());
    for i in 0..bound {
    ...
    // multiplicity for width 3 is special, as we need to assemble challenges
    {
        ...
        let t = quote! {
            let #individual_term_ident = {
                ...
                timestamp_high = timestamp_high.add_base(cs, &#memory_timestamp_high_from_sequence_idx_ident);
                denom = denom.mul(cs, &timestamp_high);

                let mut timestamp_low = #src_2_expr;
                let in_cycle_timestamp = MersenneField::allocate_constant(cs, Mersenne31Field(#in_cycle_timestamp));
